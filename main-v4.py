...
async def generate_audio(request_id: str, request: TTSRequest, stream: bool = False):
    ...
    try:
        ...
        resource_manager.active_jobs[request_id] = {
            "status": "processing", "progress": 0, "created_at": start_time
        }
    ...
    if token_count > Config.MAX_TOKEN_LIMIT:
        ...
        for i, segment in enumerate(segments):
            segment_request = TTSRequest(
                ...
                return_tokens=request.return_tokens and i == 0,
...
async def stream_audio(audio: torch.Tensor, format: AudioFormat) -> AsyncGenerator[bytes, None]:
    ...
    if format == AudioFormat.WAV:
        ...
        try:
            ...
            logger.error(f"Error during audio streaming with pydub: {str(e)}")
            buffer.seek(0)
            yield buffer.read()  # Fallback to WAV
...
async def list_endpoints():
    ...
    endpoints = {
        ...
        "/tts/preview": {
            "method": "POST",
            "description": "Generates a short audio preview (first 50 tokens) of a TTS request."
        },
        "/tts/estimate": {
            "method": "POST",
            "description": "Estimates processing time and audio duration for a TTS request."
        },
        "/status/{job_id}": {
            "method": "GET",
        ...
        "/jobs": {
            "method": "GET",
            "description": "Lists all active and recently completed jobs."
        },
        "/jobs/cancel/{job_id}": {
            "method": "POST",
        ...
        "/stats": {
            "method": "GET",
            "description": "Returns aggregated statistics about API usage."
        }
    }
...
async def get_stats():
    ...
    try:
        # Collect metrics for REQUEST_COUNT
        total_requests = 0
        ...
        # Collect metrics for REQUEST_LATENCY
        latency_sum = 0
        latency_count = 0
        ...
        completed_jobs = sum(
            1 for job in resource_manager.job_results.values() if job.get("status") == "complete"
        )
        failed_jobs = sum(
            1 for job in resource_manager.job_results.values() if job.get("status") == "failed"
        )
        ...
        total_duration = sum(
            job.get("duration", 0) for job in resource_manager.job_results.values()
...
async def list_jobs(
    ...
    return JobsResponse(
        jobs=jobs,
        total_jobs=len(resource_manager.active_jobs),
        limit=limit,
        offset=offset
    )
