...
async def generate_audio_segment(segment_id: str, segment_request: TTSRequest, model, pipeline, pack):
    ...
    output_path = Config.AUDIO_OUTPUT_DIR / f"{segment_id}.wav"
    duration = await AudioProcessor.save_audio(combined_audio, output_path)
    return {"audio_path": output_path, "duration": duration, "tokens": tokens}
...
async def generate_audio(request_id: str, request: TTSRequest, stream: bool = False):
    ...
    if token_count > Config.MAX_TOKEN_LIMIT:
        ...
        segment_ids = [f"{request_id}_{i}" for i in range(len(segments))]
        segment_tasks = [
            generate_audio_segment(segment_id, segment_request, model, pipeline, pack)
            for segment_id, segment_request in zip(segment_ids, segment_requests)
        ]
        ...
        for result in segment_results:
            rate, data = scipy.io.wavfile.read(result["audio_path"])
        ...
        result = await generate_audio_segment(request_id, request, model, pipeline, pack)
        combined_audio = torch.from_numpy(scipy.io.wavfile.read(result["audio_path"])[1]).float()
        total_duration = result["duration"]
        tokens = result["tokens"]
        os.remove(result["audio_path"])
