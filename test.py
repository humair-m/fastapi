...
async def generate_audio(request_id: str, request: TTSRequest, stream: bool = False):
    ...
    try:
        ...
        response = {
            ...
            "progress": resource_manager.active_jobs[request_id]["progress"] if request.show_progress else None
        }
...
async def process_batch(batch_id: str, items: List[TTSRequest]):
    ...
    for i, item in enumerate(items):
        try:
            ...
            processed += 1
            if any(item.show_progress for item in items):
...
async def stream_speech(
    ...
    request_id = str(uuid.uuid4())
    resource_manager.active_jobs[request_id] = {"status": "queued", "progress": 0}
    logger.info(f"Streaming TTS request with request_id: {request_id}")  # Log request_id
