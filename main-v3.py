...
class Config:
    ...
    SAMPLE_RATE = 24000
    MAX_CHAR_LIMIT = 5000
    MAX_TOKEN_LIMIT = 500  # New limit for token-based segmentation
...
class TextPreprocessor:
    ...
    def count_tokens(text: str) -> int:
        # Approximate tokens as words (split by whitespace)
...
async def synthesize_speech(
    ...
    # Check token count and segment if necessary
    ...
    if token_count > Config.MAX_TOKEN_LIMIT:
        ...
        for i, segment in enumerate(segments):
            segment_request = TTSRequest(
                ...
                return_tokens=request.return_tokens and i == 0,  # Only return tokens for first segment
                format=AudioFormat.WAV,  # Process as WAV, convert later
...
async def text_segmentation(request: TextSegmentationRequest):
    ...
    text = request.text
    max_tokens = request.max_tokens_per_segment
    segments = TextPreprocessor.segment_text_by_tokens(text, max_tokens)
    total_tokens = TextPreprocessor.count_tokens(text)
...
async def ratelimit_status():
    ...
    if not Config.ENABLE_RATE_LIMITING:
        return RateLimitStatusResponse(enabled=False, limit=None, remaining=None, reset_timestamp=None)
...
async def get_documentation():
    """
    Serves the API documentation HTML file located at /home/humair/kokoro/doc.html.
...
async def list_endpoints():
    """
    Returns a dictionary of all available API endpoints with their methods, paths, and descriptions.
    """
    endpoints = {
        "/tts": {
            "method": "POST",
            "description": "Synthesizes speech from text, segmenting if >500 tokens, returns a single audio file."
        },
        "/tts/stream": {
            "method": "POST",
            "description": "Streams audio directly without saving to disk."
        },
        "/tts/batch": {
            "method": "POST",
            "description": "Processes multiple TTS requests in a batch."
        },
        "/status/{job_id}": {
            "method": "GET",
        ...
        "/status/batch/{batch_id}": {
            "method": "GET",
            "description": "Checks the status of a batch job and its items."
        },
        "/batch/summary/{batch_id}": {
            "method": "GET",
            "description": "Summarizes a batch job’s status."
        },
        "/analyze/text": {
            "method": "POST",
            "description": "Analyzes text for potential issues."
        },
        "/voices/sample": {
            "method": "POST",
            "description": "Generates a voice sample for a specific voice."
        },
        "/pronunciation/validate": {
            "method": "POST",
            "description": "Validates a custom pronunciation."
        },
        "/audio/metadata/{filename}": {
            "method": "GET",
            "description": "Retrieves metadata for an audio file."
        },
        "/batch/download/{batch_id}": {
            "method": "GET",
            "description": "Downloads a ZIP archive of batch audio files."
        },
        "/model/info": {
            "method": "GET",
        ...
        "/text/segment": {
            "method": "POST",
            "description": "Segments text based on token limits."
        },
        "/cache": {
            "method": "GET",
            "description": "Lists cached audio files."
        },
        "/cache/{filename}": {
            "method": "DELETE",
            "description": "Deletes a cached audio file."
        },
        "/ratelimit/status": {
            "method": "GET",
        ...
        "/audio/{filename}": {
            "method": "GET",
            "description": "Retrieves an audio file."
        },
        "/audio/{filename}": {
            "method": "DELETE",
        ...
        "/voices/preview": {
            "method": "GET",
            "description": "Generates previews for all voices."
        },
        "/health": {
            "method": "GET",
        ...
        "/cleanup": {
            "method": "DELETE",
            "description": "Deletes audio files older than specified hours."
        },
        "/pronunciation": {
            "method": "POST",
            "description": "Adds a custom pronunciation."
        },
        "/pronunciations": {
            "method": "GET",
            "description": "Lists custom pronunciations."
        },
        "/pronunciations/{word}": {
            "method": "DELETE",
            "description": "Deletes a custom pronunciation."
        },
        "/preprocess": {
            "method": "POST",
            "description": "Preprocesses text (lowercase, removes special characters, expands abbreviations)."
        },
        "/metrics": {
            "method": "GET",
        ...
        "/config": {
            "method": "GET",
            "description": "Returns API configuration."
        },
        "/jobs/cancel/{job_id}": {
            "method": "POST",
        ...
        "/doc": {
            "method": "GET",
            "description": "Serves the API documentation HTML file."
        },
        "/endpoints": {
            "method": "GET",
            "description": "Returns a dictionary of all available API endpoints."
        }
    }
...
@app.get("/stats")
async def get_stats():
    total_requests = sum(REQUEST_COUNT._metrics.values()).get() or 0
    ...
    avg_processing_time = REQUEST_LATENCY._metrics.get(("tts",), {}).get("sum", 0) / max(1, REQUEST_COUNT._metrics.get(("tts",), {}).get("count", 1))
    total_duration = sum(job.get("duration", 0) for job in resource_manager.job_results.values() if job.get("status") == "complete")
    cache_hits = audio_cache.hits if hasattr(audio_cache, "hits") else 0  # Extend TTLCache if needed
    cache_misses = audio_cache.misses if hasattr(audio_cache, "misses") else 0
...
async def tts_preview(request: TTSRequest, background_tasks: BackgroundTasks):
