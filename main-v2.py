...
class TextSegmentationRequest(BaseModel):
    ...
    max_chars_per_segment: int = Field(default=Config.MAX_CHAR_LIMIT, ge=100, le=Config.MAX_CHAR_LIMIT)
...
async def analyze_text(request: TextAnalysisRequest):
    ...
    estimated_duration = char_count / 200 * 60 / Config.SAMPLE_RATE  # Rough estimate
...
async def text_segmentation(request: TextSegmentationRequest):
    ...
    text = request.text
    max_chars = request.max_chars_per_segment
    ...
    for sent in doc.sents:
        ...
        if len(current_segment) + len(sent_text) <= max_chars:
